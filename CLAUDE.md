...
- Follow-on and recycled capital earn each outcome bucket's own multiple divided by `foStepUp` (a step-up discount for entering later than the initial check), allocated across buckets by `allocate()` in engine.js — a blend of an even split and a hindsight/winner-weighted split, controlled by `foSkill` (0–1). `failYear` holds failed positions at cost in the NAV/RVPI/TVPI-over-time chart until that year instead of writing them off immediately; it never affects DPI or IRR.
